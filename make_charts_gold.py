...
# ---------------------------------------------------------------
# Chart 1: cena PAXG/USD (złoto, token na Krakenie) (30 dni) + anomalie z anomalies() na 1h zwrotach
...
ax.plot(t_h / 24.0, close, color=COL_REAL, lw=1.0, label='PAXG/USD (złoto, token na Krakenie), świece 1h (Kraken)', zorder=2)
...
ax.set_ylabel('Cena PAXG/USD (złoto, token na Krakenie)')
ax.set_title('PAXG/USD (złoto), 30 dni (720 świec 1h) — anomalie wykryte przez TIMDR anomalies()', fontsize=12)
ax.legend(fontsize=8.5, loc='upper left')
fig.tight_layout()
fig.savefig('chart_gold_anomalies.png', dpi=150)
print('saved chart_gold_anomalies.png')
...
fig.suptitle('Prognoza zmienności PAXG/USD (złoto, token na Krakenie) (kauzalny walk-forward, 182 punkty) — co działa, co nie', fontsize=12)
fig.tight_layout()
fig.savefig('chart_gold_vol_forecast.png', dpi=150)
print('saved chart_gold_vol_forecast.png')
